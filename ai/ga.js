...
function GA(intelligence) {
    let SOLUTIONS = window.getSolutions();
    ...
    class IndividualSolution {
        ...
        crossover (other) {
            ...
            let childs = [new IndividualSolution(network1), new IndividualSolution(network2), new IndividualSolution(network3)];
    ...
    if(intelligence) {
        while (pop.length < POP_SIZE) {
            pop.push(new IndividualSolution(intelligence))
        }
    } else {
        while (pop.length < POP_SIZE) {
